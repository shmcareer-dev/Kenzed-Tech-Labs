/**
 * Company-wide facts, navigation, and contact details.
 * Edit here to update every page at once.
 */

export const site = {
  name: "Kenzed Tech Lab",
  tagline: "Agentic AI · ML · Software",
  legalName: "Kenzed Tech Lab",
  description:
    "Premium agentic AI, machine learning and software development company engineering production-grade intelligent systems.",
  /** Public origin. Override with NEXT_PUBLIC_SITE_URL in .env.local. */
  url: process.env.NEXT_PUBLIC_SITE_URL ?? "https://kenzed.in",
  email: "kenzedtechlab@gmail.com",
  phone: "+91-7699002237",
  foundedTeamSize: 25,
} as const;

export type NavLink = { href: string; label: string };

/** Primary header navigation. */
export const navLinks: NavLink[] = [
  { href: "/services", label: "Services" },
  { href: "/technology", label: "Technology" },
  { href: "/infrastructure", label: "Infrastructure" },
  { href: "/process", label: "Process" },
  { href: "/about", label: "About" },
];

export type Location = {
  kind: string;
  city: string;
  street: string;
  postalCode: string;
  region: string;
  country: string;
  note?: string;
};

export const locations: Location[] = [
  {
    kind: "Engineering Centre",
    city: "Durgapur",
    street: "Rajbandh",
    postalCode: "713212",
    region: "West Bengal",
    country: "IN",
    note: "15,000 sq ft facility",
  },
  {
    kind: "Liaison Office",
    city: "Kolkata",
    street: "Action Area 1, Tower 10, Sankalpa Newton",
    postalCode: "700156",
    region: "West Bengal",
    country: "IN",
  },
];

export type HeroStat = { value: number; suffix: string; label: string };

/** Animated counters over the 3D hero. */
export const heroStats: HeroStat[] = [
  { value: 25, suffix: "+", label: "AI, software & design specialists" },
  { value: 15000, suffix: " sq ft", label: "Dedicated development facility" },
  { value: 24, suffix: "×7", label: "Operations & in-house AI compute" },
  { value: 2, suffix: "", label: "Locations — Durgapur & Kolkata" },
];

/** Icon keys map to <TrustIcon /> in components/ui/icons.tsx */
export const trustSignals = [
  { icon: "shield", label: "Enterprise-grade & secure" },
  { icon: "layers", label: "Full-stack under one roof" },
  { icon: "cpu", label: "Owned GPU compute" },
  { icon: "bolt", label: "24×7 uninterrupted power" },
  { icon: "globe", label: "Delivered worldwide" },
] as const;
