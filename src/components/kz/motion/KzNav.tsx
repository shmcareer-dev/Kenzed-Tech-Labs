...
export interface KzScrollSpyItem {
  /** id of the section element this entry tracks. */
  id: string;
  label: string;
}

export interface KzScrollSpyProps {
  items: KzScrollSpyItem[];
  /** Sticky-header allowance, in pixels. */
  offsetTop?: number;
  /** Numbers the entries 01, 02, … */
  numbered?: boolean;
  ariaLabel?: string;
  className?: string;
  style?: CSSProperties;
...
/**
 * Table of contents driven by an IntersectionObserver over the real sections.
 * The active marker is a hairline bar moved with translate + scaleY, so a
 * one-line and a three-line entry both get an exactly fitting indicator without
 * a single layout-triggering property.
 */
export function KzScrollSpy({
  items,
  offsetTop = 96,
  numbered = true,
  ...
  const measure = useCallback(() => {
    ...
    if (!nav || !link) return;
    const base = Number.parseFloat(nav.style.getPropertyValue("--kzss-h")) || link.offsetHeight;
    if (!nav.style.getPropertyValue("--kzss-h")) nav.style.setProperty("--kzss-h", `${base}px`);
    nav.style.setProperty("--kzss-y", `${link.offsetTop}px`);
    nav.style.setProperty("--kzss-s", `${base > 0 ? link.offsetHeight / base : 1}`);
  }, [activeId]);
  ...
  return (
    <>
      <KzNavStyles />
      <nav ref={navRef} aria-label={ariaLabel} className={`kzss ${className}`.trim()} style={style}>
        <span className="kzss-bar" aria-hidden="true" />
        <ol className="kzss-list">
          {items.map((item, index) => (
            <li key={item.id}>
              ...
              >
                {numbered && <span className="kzss-num">{String(index + 1).padStart(2, "0")}</span>}
